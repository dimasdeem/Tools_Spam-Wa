...
{biru}╚═╗{W}├─┘├─┤│││  {biru}║║║{W}├─┤├─┤ │ └─┐{merah}├─┤{kuning}├─┘{Hijau}├─┘
{biru}╚═╝{W}┴  ┴ ┴┴ ┴  {biru}╚╩╝{W}┴ ┴┴ ┴ ┴ └─┘{merah}┴ ┴{kuning}┴  {Hijau}┴{kuning} V1
{abu}-----------------------------------------
{putih}[{biru}•{putih}] {biru}Author {putih}: HαckͥDeͣeͫm
{putih}[{biru}•{putih}] {biru}Youtub {putih}: HerDim.Net

{W}[{Y}•{W}] Jangan {W}Lupa Subscribe Yaa
{W}[{Y}•{W}] Ip Anda{R}:{Y} {ip}
{W}[{Y}•{W}] Waktu/Jam{R}:{Y} {localtime}

""")
autoketik(f"{W}[{R}• {kuning}•{Hijau}•{W}]{W}[{Y}•{W}] {biru}Nomor Target")
nomor = input(f"{W}[{R}• {kuning}•{Hijau}•{W}]{W}[{Y}•{W}] {abu}Exp:08xx{W}: ")
jum = int(input(f"{W}[{R}• {kuning}•{Hijau}•{W}]{W}[{Y}•{W}] {biru}Jumlah {W}: "))
for i in range(jum):
    ...
    print (f"{W}[{R}• {kuning}•{Hijau}•{W}][{G}✓{W}] Succes Sended Brutal WA")
